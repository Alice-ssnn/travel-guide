...
### 2.1 API 架构设计
- **密钥管理**: 
  - 开发环境: 使用环境变量 (VITE_GOOGLE_MAPS_API_KEY)
  - 生产环境: 后端代理API保护密钥安全
  - 用量监控: 实施配额监控和预算告警
- **按需加载**: 
  - 动态加载Google Maps JavaScript API (`https://maps.googleapis.com/maps/api/js`)
  - 延迟加载，仅在需要地图的页面初始化
  - 加载失败时自动重试机制
- **错误处理**: 
  - 网络失败: 显示静态地图图片作为备胎
  - API配额超限: 降级到开源地图库 (Leaflet + OpenStreetMap)
  - 浏览器不支持: 提供坐标文本描述替代方案
...
### 2.4 离线地图策略
1. **静态地图备胎**: 
   - 为每个行程日生成静态地图截图（使用Google Static Maps API）
   - 离线时显示带标记和路线的静态图片
   - 轻量级（每张图片<100KB），预缓存所有15天
2. **矢量切片缓存**:
   - 使用开源解决方案（Leaflet + OpenStreetMap）
   - 预缓存瑞士和法国关键区域的地图切片
   - 用户可选择下载特定城市/区域的地图数据
3. **动态地图降级**:
   - 检测离线状态，自动切换到静态地图模式
...
1. **手动录入**: 已有行程的详细数据整理
2. **API 补充**: 
   - **Google Places API**: 获取地点详细信息（评分、营业时间、照片）
   - **地理编码**: 地址转坐标，坐标转地址
   - **增量更新**: 仅请求缺失信息，缓存API响应
   - **离线支持**: API响应缓存在IndexedDB中
3. **用户贡献**: 
   - 允许用户添加自定义活动
   - 编辑现有活动信息（标记为用户修改）
   - 用户数据与原始数据隔离存储
